import numpy as np
from pyevals.exceptions import MetricValueError
from .utils import simple_error
from .utils import equal_lengths
from .utils import check_type
from .utils import input_regression
...
def rsquared_score(actual, predicted):
    ...
    if len(actual) == 0:
        raise MetricValueError('There are currently no values in actual.')
    if len(predicted) == 0:
        raise MetricValueError('There are currently no values in predicted.')
    # for i in range(len(actual)):
    #     if (actual[i] < 0).any() or (predicted[i] < 0)():
    #         raise MetricValueError('There cannot be any negative values to calculate r2_score')
    ...
    sse = ((actual - predicted) ** 2).sum(dtype=np.float)
    sse_nonzero = sse != 0  # To check for only non-zero values as input
    sst = ((actual - np.average(actual)) ** 2).sum(dtype=np.float)
...
def mean_absolute_percentage_error(actual, predicted):
    ...
    if len(actual) == 0 or len(predicted) == 0:
        raise MetricValueError('There are currently no values in your actual or predicted')
...
def adj_rsquared_score(actual, predicted, n = 0, p = 0):
    """
    :Params:
    --------
    actual = Your y_test
